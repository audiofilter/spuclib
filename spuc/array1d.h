...
*/
//! \file
//! \brief Templated one-dimensional, numerical array which
...
class array1d {
 ...
 public:
  ...
  array1d<T>& set_size(int m, bool f) { return (set_size(m)); }
  array1d<T>& newsize(int m, bool f) { return (set_size(m)); }
  array1d<T>& newsize(int m) { return (set_size(m)); }
  array1d<T>& set_size(int m) {
    if (dim1() == m) return *this;
    destroy_();
    initialize_(m);
    return *this;
  }
