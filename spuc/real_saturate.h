...
//! \ingroup real_template_functions misc
template <>
inline long real_saturate(long in, long bits) {
...
template <>
inline int real_saturate(int in, long bits) {
...
template <>
inline double real_saturate(double in, long bits) {
  int low_mask = ((1 << (bits - 1)) - 1);
...
template <>
inline float real_saturate(float in, long bits) {
  int low_mask = ((1 << (bits - 1)) - 1);
