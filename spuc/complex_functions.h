...
//! Complex value (0,1)
template <typename T>
inline complex<T> complexj(void) {
  return (complex<T>(0, 1));
}
//! Conjugate
template <typename T>
inline complex<T> conjugate(complex<T> x) {
  complex<T> y;
  y.re = x.re;
  y.im = -x.im;
  return y;
...
template <typename T>
inline T magnitude_squared(complex<T> x) {
  ASP_C(hcmult_count)
  ASP_C(hcadd_count)
...
template <typename T>
inline complex<double> normalized(complex<T> x) {
  T y;
...
template <typename T>
inline complex<T> real_mult(complex<T> r, complex<T> l) {
  ASP_C(hcmult_count)
  ASP_C(hcadd_count)
  complex<T> z;
...
template <typename T>
inline complex<T> reals_only_mult(complex<T> r, complex<T> l) {
  ASP_C(rmult_count)
  complex<T> z;
  z.re = (r.re * l.re);
  z.im = 0;
...
template <typename T>
inline T approx_mag(complex<T> x) {
  T xa = ABS(x.re);
  T ya = ABS(x.im);
...
template <typename T>
inline double arg(const complex<T> x) {
  double TMPANG;
  if (real(x) == (T)0) {
    if (imag(x) < (T)0)
      return (3.0 * PI / 2.0);
...
#endif
template <typename T>
inline double calc_angle(const complex<T> x) {
  double TMPANG;
  if (real(x) == (T)0) {
    if (imag(x) < (T)0)
      return (3.0 * PI / 2.0);
...
template <typename T>
inline complex<double> rational(complex<T> l) {
  return (complex<double>((double)l.re, (double)l.im));
}
template <typename T>
inline complex<double> reciprocal(complex<T> x) {
