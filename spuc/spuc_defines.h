...
/*! \mainpage
 ...
 <P><B>Motivation:Why use C++ for DSP Simulation/Modeling?</B>
         ...
         algorithms. While the latter are well suited for modeling "hardwired"
 DSP blocks
         with rather simplistic dataflows, they are very inefficient for
 modeling the control logic that is often required in todays DSP applications
 (such as ADSL or V.34 modems).
         while C is well suited to control logic type structures (if,else, case,
 etc.), it is not highly desireable for manipulation of complex
         ...
         <P><B>Objective</B></P>
         <P>The objective of DSP Templates is to provide DSP Algorithm designer
 with simple, efficient and reusable DSP building block objects. Thus allowing a
 transition from System design to implementation in either programmable DSP
 chips or hardwired DSP logic. While Matlab is perhaps the most useful available
 tool for this purpose, it can be quite slow for simulation and it favors a
 matrix/block based approach rather than the sample by sample simulations that
 are often most useful for communications systems design. Also Matlab is
 generally awkward or inefficient when dealing with several interactive feedback
 loops where C/C++ is perhaps the most useful environment. For bit-accurate
 simulations (for VLSI design) C/C++ generally outperforms and is easier to
 manipulate than Matlab or other GUI-based tools.</P>
         <P>This Class Library</P>
         <ul> <li> 1) basic building blocks such as complex data types,
 Fixed-bit width integer classes, pure-delay blocks, etc.
         <li> 2) Basic DSP building blocks such as FIR, IIR, Allpass, Running
 Average, Lagrange interpolation filters, NCO, Cordic rotator.
         <li> 3) Several communications functions such as timing, phase and
 frequency discriminators for BPSK/QPSK signals.
         <li> 4) Other miscellaneous DSP/Communications related
 functions/classes.
     ...
     <li> 8) Capitalized and uppercase class names are classes not originally in
 DSP Templates
         </ul>

         <P>The classes are designed so that they can be used in a simple
 straight forward manner. For example, a FIR would be initialized with its tap
 weights and then simply a member function would be called every time a sample
 is input or an output is required.</p>

         <P>Before discussing why C++ is better than C for DSP programming.
 Let's look at some of the pros and cons
         for C++ vs. the alternatives.</p>

         <P><B>Pros</B></p>
         <ul><li>  The common alternatives are very costly (Alta's SPW, Synopsys
 Cossap etc.)
         <li>    Long learning times/training not required for C++ programmers.
         <li>   For complex simulations, C++ is more efficient and yields
 shorter run times.
         <li>   Ideal for modeling a combination of control logic and hardwired
 DSP blocks.
         ...
         <P>Please see links such as those below for why to use C++ over C in
 general.</p>
         <BR><A HREF="http://www.cantrip.org/cpp.html">Nathan Myers C++
 Articles</A>
         <BR><A HREF="http://math.nist.gov/pozo/c++class/">C++ Programming for
 Scientists Course (PDF) from NIST</A>
         <BR><A HREF="http://math.nist.gov/pozo/">Various C++ Math classes from
 Roldan Pozo at NIST</A>
         ...
         <BR>For example, we can define a complex data type based on templates.
 Then this class can be used whether we need floating point (double), integer,
 or user defined fixed bit width data types.This not only requires less code to
 document/debug, but also through operator overloading we can make the code much
 easier to read and potentially make the look and feel very close to a Hardware
 Description Language (HDL)such as Verilog or VHDL. Also this style makes it
 much easier to change your programs data types (from floating point -> integer
 -> fixed bit width for example), without having to change every line of code.
         <P><B>C++ allows for much better interfaces between classes or DSP
 blocks.</B>
         ...
         <P><B>Inheritance</B>
         <BR>Inheritance helps you to re-use code.You can derive new types or
 classes from an old one and make changes only where you need them. This
 promotes code re-use.
         ...
         <LI>Data abstraction and hiding allows changes to be localized to one
 section.</LI>
         ...
         Functions and variables can be localised to each block (hiding
 complexity) in the same way that Verilog /VHDL does.
         Easier to have a hierarchical structure.
         Classes can be instantiated in same manner as in a HDL (C makes this
 difficult).
         New chip designs can inherit subblocks or a large section from an old
 design. Only the new or changed blocks need to be coded. Also the interfaces to
 the chip can remain the same if desired. When classes are designed in this
