...
LINEAR:
  ...
  roots[index++] = complex<float_type>(r, 0.0);
...
QADRTIC:
  ...
  if (0.0 < (q - (p * p / 4.0))) { /* Two complex roots */
    s = sqrt(q - (p * p / 4.0));
    roots[index] = complex<float_type>(-p / 2.0, s);
    roots[index + 1] = complex<float_type>(-p / 2.0, -s);
  } else { /* Two real roots */
    s = sqrt(((p * p / 4.0)) - q);
    if (p < 0.0)
      roots[index] = complex<float_type>(-0.5 * p + s, 0.0);
    else
      roots[index] = complex<float_type>(-0.5 * p - s, 0.0);

    roots[index + 1] = complex<float_type>(real(q / roots[index]), 0.0);
  }
  index += 2;
