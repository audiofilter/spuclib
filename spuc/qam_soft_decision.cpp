...
void qam16_soft_decision(complex<long> data_in, long soft_decision_level, smart_array<long>& viterbi_input)
{
  ...
  else 		          viterbi_input[1] = soft_decision_level - real(data_in);
  viterbi_input[2] = imag(data_in);
  if (imag(data_in) < 0) viterbi_input[3] = soft_decision_level + imag(data_in);
  else 		          viterbi_input[3] = soft_decision_level - imag(data_in);
}
//--------------------------------------------------------------------------
...
void qam64_soft_decision(complex<long> data_in, long soft_decision_level, smart_array<long>& viterbi_input)
{
  // real part
  viterbi_input[0] = real(data_in);
  if (real(data_in) < 0) viterbi_input[1] = 2*soft_decision_level + real(data_in);
  else 		          viterbi_input[1] = 2*soft_decision_level - real(data_in);
  if (ABS(real(data_in)) < 2*soft_decision_level) {
	  if (real(data_in) < 0) viterbi_input[2] = -soft_decision_level - real(data_in);
	  else 		          viterbi_input[2] = -soft_decision_level + real(data_in);
  } else {
	  if (real(data_in) < 0) viterbi_input[2] = 3*soft_decision_level + real(data_in);
	  else 		          viterbi_input[2] = 3*soft_decision_level - real(data_in);
  }
  // imaginary part
  viterbi_input[3] = imag(data_in);
  if (imag(data_in) < 0) viterbi_input[4] = 2*soft_decision_level + imag(data_in);
  else 		          viterbi_input[4] = 2*soft_decision_level - imag(data_in);
  if (ABS(imag(data_in)) < 2*soft_decision_level) {
	  if (imag(data_in) < 0) viterbi_input[5] = -soft_decision_level - imag(data_in);
	  else 		          viterbi_input[5] = -soft_decision_level + imag(data_in);
  } else {
	  if (imag(data_in) < 0) viterbi_input[5] = 3*soft_decision_level + imag(data_in);
	  else 		          viterbi_input[5] = 3*soft_decision_level - imag(data_in);
  }
}
} // namespace SPUC
